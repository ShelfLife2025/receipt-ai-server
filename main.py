...
MERGE_WITH_NEXT_MAX_HEAD_TOKENS = int(os.getenv("MERGE_WITH_NEXT_MAX_HEAD_TOKENS", "1"))
MERGE_WITH_NEXT_MAX_HEAD_LEN = int(os.getenv("MERGE_WITH_NEXT_MAX_HEAD_LEN", "7"))
...
STORE_WORDS_RE = re.compile(r"\b(publix|wal[-\s]*mart|walmart|target|costco|kroger|aldi|whole\s+foods|trader\s+joe'?s|wm\s*supercenter)\b", re.IGNORECASE)
...
_JUNK_EXACT_LINES = {"t", "f", "tf", "t f", "ft", "tt", "ff", "grocery", "groceries"}
...
def title_case(s: str) -> str:
    ...
    return " ".join(cap_word(w, i == 0) for i, w in enumerate(words)).strip()
...
ABBREV_TOKEN_MAP: Dict[str, str] = {
    "pblx": "publix", "publx": "publix", "pub": "publix", "pbl": "publix", "pbx": "publix", "gg": "good and gather",
    "chees": "cheese", "ches": "cheese", "rte": "rte", "sar": "sargento", "arts": "artisan", "blnd": "blends",
    "parm": "parmesan", "nesp": "nespresso", "pke": "pike", "sprng": "spring", "wht": "white", "sdl": "seedless",
    "bg": "bagels", "swr": "swirl", "swrl": "swirl", "crm": "cream", "chs": "cheese", "whp": "whipping",
    "hvy": "heavy", "bttr": "butter", "marg": "margarine", "yog": "yogurt", "veg": "vegetable", "org": "organic",
    "grd": "ground", "bf": "beef", "chk": "chicken", "ckn": "chicken", "chkn": "chicken", "brst": "breast",
    "bnls": "boneless", "sknls": "skinless", "flr": "flour", "pdr": "powdered", "sug": "sugar", "brn": "brown",
    "van": "vanilla", "ext": "extract", "vngr": "vinegar", "alc": "alcohol", "iso": "isopropyl", "isoprop": "isopropyl",
    "isopropyl": "isopropyl", "prm": "premium", "grl": "grilled", "pta": "potato", "rst": "roast", "rstd": "roasted",
    "ov": "oven", "jd": "jack daniel's", "tenn": "tennessee", "hny": "honey", "sh": "sharp", "chd": "cheddar", "cut": "cut",
}
PHRASE_MAP: Dict[str, str] = {
    "half and half": "half-and-half", "h and h": "half-and-half", "hnh": "half-and-half",
    "heavy whipping cream": "heavy whipping cream", "cream cheese": "cream cheese", "sour cream": "sour cream",
    "ground beef": "ground beef", "chicken breast": "chicken breast",
    "boneless chicken breast": "boneless chicken breast", "boneless skinless chicken breast": "boneless skinless chicken breast",
    "olive oil": "olive oil", "vegetable oil": "vegetable oil", "all purpose flour": "all-purpose flour",
    "powdered sugar": "powdered sugar", "brown sugar": "brown sugar", "vanilla extract": "vanilla extract",
    "apple cider vinegar": "apple cider vinegar", "balsamic vinegar": "balsamic vinegar",
}
...
STORE_TOKEN_MAPS: Dict[str, Dict[str, str]] = {
    "publix": {
        "wdg": "wedge", "wdge": "wedge", "dcd": "diced", "slt": "salted", "unslt": "unsalted", "it": "italian",
        "itl": "italian", "frsh": "fresh", "pk": "pack", "pck": "pack", "pks": "packs", "swt": "sweet",
        "pln": "plain", "sprd": "spread", "chz": "cheese", "mnst": "muenster", "mnstr": "muenster", "moz": "mozzarella",
        "mzzrl": "mozzarella", "parm": "parmesan", "rg": "regular", "lg": "large", "sm": "small",
    },
    "target": {"gg": "good and gather"},
    "walmart": {}, "aldi": {}, "costco": {}, "kroger": {}, "whole foods": {}, "trader joe's": {},
}
GLOBAL_TOKEN_MAP: Dict[str, str] = {
    "hvy": "heavy", "whp": "whipping", "crm": "cream", "chs": "cheese", "chees": "cheese", "bttr": "butter",
    "marg": "margarine", "yog": "yogurt", "org": "organic", "veg": "vegetable", "grd": "ground", "bnls": "boneless",
    "sknls": "skinless", "brst": "breast", "chk": "chicken", "ckn": "chicken", "chkn": "chicken", "bf": "beef",
    "blk": "black", "pepr": "pepper", "grn": "green", "chp": "chip", "mffn": "muffin", "sdls": "seedless",
    "orig": "original", "prog": "progresso", "tus": "tuscan", "bea": "beans", "ches": "cheese", "ex": "extra",
    "srp": "sharp", "ba": "bagels", "belg": "belgioioso", "alc": "alcohol", "iso": "isopropyl", "isoprop": "isopropyl",
    "jd": "jack daniel's", "tenn": "tennessee", "rte": "rte",
}
FORCED_FALLBACK_MAP: Dict[str, str] = {
    "snk": "snack", "sn": "snack", "chs": "cheese", "chz": "cheese", "wt": "weight", "ct": "count", "pk": "pack",
    "pck": "pack", "reg": "regular", "lrg": "large", "lg": "large", "sm": "small", "frz": "freezer", "frsh": "fresh",
    "dcd": "diced", "swt": "sweet", "pln": "plain", "brd": "bread",
}
ALLOWED_SHORT_TOKENS = {"oz", "lb", "lbs", "g", "kg", "ml", "l", "xl", "xxl", "bf", "rte"}
...
_ENRICH_SCORE_STOPWORDS = {
    "the", "and", "or", "of", "a", "an", "with", "for", "pack", "ct", "count", "oz", "lb", "lbs", "g", "kg", "ml", "l",
...
def _pending_add(store_hint: str, raw_line: str, cleaned: str, expanded: str) -> None:
    ...
    rec = _PENDING.get(key) or {
        "store_hint": store_hint, "raw_examples": [], "cleaned": cleaned, "expanded": expanded, "count": 0, "last_seen": 0,
    }
...
def _tokenize_for_score(s: str) -> List[str]:
    return [t for t in dedupe_key(s).split() if t and t not in _ENRICH_SCORE_STOPWORDS]
...
async def _catalog_best_match(name: str, store_hint: str, category: str, budget: ReqBudget) -> Optional[Tuple[str, float, str, str]]:
    ...
    for v in variants:
        ...
        qvars.append(v)
    params_base = {
        "search_simple": 1, "action": "process", "json": 1, "page_size": OFF_PAGE_SIZE,
...
async def enrich_full_name(raw_line: str, cleaned_name: str, expanded_name: str, store_hint: str, category: str, budget: ReqBudget) -> Tuple[str, str, float, str]:
...
TOTAL_MARKER_RE = re.compile(r"\b(sub\s*total|subtotal|tax|balance|amount\s+due|total\s+due|grand\s+total)\b", re.IGNORECASE)
...
def looks_like_item_name(s: str) -> bool:
    ...
    if len(s2) < 3:
        return False
    low = s2.lower()
    bad = ("subtotal", "total", "tax", "visa", "mastercard", "amex", "cash", "change", "balance", "tender", "refund", "payment", "store", "auth", "trace", "acct", "circle", "savings")
...
def _should_merge_with_next(head: str, tail: str, next_support: str) -> bool:
    ...
    if not head_toks:
        return False
    is_store_only = all(t in _STORE_TOKENS for t in head_toks)
    short_lead = (len(head_toks) <= MERGE_WITH_NEXT_MAX_HEAD_TOKENS and len(head_key) <= MERGE_WITH_NEXT_MAX_HEAD_LEN) or is_store_only
    if not short_lead:
        return False
    ...
    if not next_support:
        return False
    return bool(_is_weight_or_unit_price_line(next_support) or _is_price_like_line(next_support) or WEIGHT_ONLY_RE.match(next_support) or _line_has_embedded_price(tail))
        ...
        if len(split_parts) > 1:
            for part in split_parts:
                ...
                if not part_clean or not _has_valid_item_words(part_clean):
                    ...
                    continue
                candidates.append(Candidate(raw_line=part, cleaned_line=part_clean, qty_hint=1))
            i += 1
            continue
...
@app.post("/parse-receipt/", response_model=List[ParsedItem])
async def parse_receipt(request: Request, file: Optional[UploadFile] = File(None), image: Optional[UploadFile] = File(None), only_food: bool = Query(True), debug: bool = Query(False)):
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"error": f"OCR failed: {str(e)}", "hint": "Check GOOGLE_APPLICATION_CREDENTIALS_JSON / GOOGLE_APPLICATION_CREDENTIALS"})
        ...
        for it in items:
            ...
            if ENRICH_SEM:
                async with ENRICH_SEM:
                    enriched, _src, _score, _q = await enrich_full_name(raw_line=raw_line, cleaned_name=name_cleaned, expanded_name=expanded, store_hint=store_hint, category=category, budget=budget)
            else:
                enriched, _src, _score, _q = await enrich_full_name(raw_line=raw_line, cleaned_name=name_cleaned, expanded_name=expanded, store_hint=store_hint, category=category, budget=budget)
...
@app.post("/parse-receipt-debug/")
async def parse_receipt_debug(request: Request, file: Optional[UploadFile] = File(None), image: Optional[UploadFile] = File(None), only_food: bool = Query(True), debug: bool = Query(True)):
    upload = file or image
    if upload is None:
        raise HTTPException(status_code=422, detail="Missing receipt file field (expected multipart 'file' or 'image').")
    raw = await upload.read()
    ...
    for c in candidates:
        ...
        if ENABLE_NAME_ENRICH and budget.remaining() > 0.9 and (always_off or _needs_official_name(expanded)):
            enriched, source, score, query_used = await enrich_full_name(raw_line=c.raw_line, cleaned_name=_clean_line(nm), expanded_name=expanded, store_hint=store_hint, category=category_guess, budget=budget)
        ...
        if only_food and category != "Food":
            continue
        parsed.append({"name": final_name, "quantity": int(max(1, qty)), "category": category, "image_url": _image_url_for_item(base_url, final_name)})
...
async def get_product_image(name: str = Query(...), upc: Optional[str] = Query(None), product_id: Optional[str] = Query(None)):
    ...
    if result:
        ...
        _trim_caches_if_needed()
        return Response(content=img_bytes, media_type=ctype)
    tiny_bytes = base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGNgYAAAAAMAASsJTYQAAAAASUVORK5CYII=")
